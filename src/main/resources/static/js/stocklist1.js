layui.use(['form', 'table', 'miniPage', 'element', 'jquery'], function () {
    ...
    table.on('tool(currentTableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            ...
            $.post("http://localhost:8080/delstock", {
                code:data.code,
                //Lu 4.12 up END
        ...
        if (obj.event === 'update') {
            // 股票实时分析图
            sessionStorage.setItem("stockcode",data.code)
            window.parent.location.href = "/page/adminindex#//page/updateStockInfo"
        }
        if (obj.event === 'add') {
            sessionStorage.setItem("stockcode",data.code)
            window.parent.location.href = "/page/adminindex#//page/addStockNum"
        }
    });
});
