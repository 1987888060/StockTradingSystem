layui.use(['form', 'table', 'miniPage', 'element', 'jquery'], function () {
    ...
    table.render({
        id: "tableone",
        elem: '#currentTableId',
        url: 'http://localhost:8080/mybuyinglist',
        toolbar: '#toolbarDemo',
        ...
        cols: [[
            ...
            {field: 'num', width: 140, title: '剩余数量'},
    ...
    $('#sear').on('click', function () {
        ...
        if (send_name == '') {
            ...
            table.reload('tableone', {
                method: 'post',
                url: 'http://localhost:8080/mysellinglist',
                where: {
                    'daima': send_name
                },
                page: {
    ...
    table.on('tool(currentTableFilter)', function (obj) {
        ...
        if (obj.event === 'disbuy') {
            console.log(data);
            $.post("http://localhost:8080/disbuy", {
                id:data.id,
                //Lu 4.12 up END
                ...
                // 函数回调
                console.log(resu)
                window.parent.location.href = "/page/index#//page/mybuying"
            })
            return false;
        }
        if (obj.event === 'info'){
